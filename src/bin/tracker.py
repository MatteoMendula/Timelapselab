...
from ultralytics.yolo.utils import DEFAULT_CFG, LOGGER, SETTINGS, callbacks, colorstr, ops, yaml_load
...
class Tracker(BaseBin):
    ...
    def run(self,
            ...
            imgsz=(2160, 3840),  # inference size (height, width)
        ...
        exp_name = '{}/{}'.format(self.detector.name, tracking_method)
        save_dir = increment_path(Path(project) / exp_name, exist_ok=exist_ok)  # increment run
        (save_dir / 'tracks' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir

        # Load model
        device = select_device(device)
        model = AutoBackend(self.detector.yolo_weights_path, device=device, dnn=dnn, fp16=half)
        stride, names, pt = model.stride, model.names, model.pt
        # model = self.detector.model
        # stride, names, pt = model.model.stride, model.model.names, model.model.model.pt
        ...
        tracking_config = 'src/trackers/{}/configs/{}.yaml'.format(tracking_method, tracking_method)
        tracker_list = []
        for i in range(bs):
            tracker = create_tracker(tracking_method, tracking_config, self.reid_weights, device, half)
        ...
        for frame_idx, batch in enumerate(dataset):
            ...
            # Apply NMS
            with dt[2]:
                pred = non_max_suppression(preds, conf_thres, iou_thres,
                                           self.classes_to_track, agnostic_nms, max_det=max_det)
            print(f'pred: {pred}')
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                annotator = Annotator(blurred_image, line_width=line_thickness, example=str(names))
                ...
                if det is not None and len(det):
                    ...
                    if len(outputs[i]) > 0:
                        ...
                        for j, (output) in enumerate(outputs[i]):
                            ...
                            self.tracked_objects_dict[int(cls)].append(id)
                            ...
                            if save_vid or save_crop or show_vid:  # Add bbox/seg to image
                                ...
                                if save_crop:
                                    ...
                                    save_one_box(torch.tensor(bbox), imc,
